...
- ↑↓持续输入且实际移动才循环攀爬；受实体墙阻挡时暂停第0帧。身体完全离开检测区后松手下落。
- 攀爬中按跳跃离链，`jump` 从第0帧重新播放，可配合左右键。
- 下爬接触地面自动放手。抓链会清除之前的坠落累计，离链后重新计量。
- 攀爬中←→允许横移；身体与检测区重叠时只横移保持第0帧、松键悬停。四个方向均在身体完全离开检测区后自动放手，保持jump末帧并恢复重力，不施加起跳冲量。
...
玩家根节点的 Climbing 分组：Climb Speed=90（上下速度，像素/秒）、Climb Horizontal Speed=35（左右速度，像素/秒）。旧的Climb Horizontal Limit已移除：移动和脱离统一只看身体碰撞形状与锁链检测形状是否重叠，不再限制手部、角色原点或半宽距离。左右活动区通过锁链Detection Width调整，上下通过Length及两个Margin调整。两份脚本必须配套替换，新玩家调用锁链的overlaps_character()。

抓链、保持攀附和松手统一调用Shape2D.collide比较当前世界变换下的实际碰撞形状；支持多个启用形状，排除禁用形状，遵守锁链Monitoring和Collision Mask。移动后立即重新比较，不依赖Area2D上一物理帧的重叠缓存。身体仍有任何形状重叠便可继续四向移动，完全无重叠才松手。实体墙/地面仍阻挡运动；向下接触地面仍恢复普通站立。

ClimbGrip只用于初次抓取时的横向画面对齐，不参与攀爬边界判定。抓链仍要求有ClimbGrip、climb帧、抓取冷却结束，且横向对齐不穿墙。按住X跳进范围不会自动抓住，需松开后再次按X。只看到美术重叠不代表碰撞体相交。
...
把 `exploration_camera.gd` 挂在已有Camera2D上，推荐Camera2D为玩家直接子节点；Player属性拖入玩家根节点。只保留一个启用的游戏Camera2D。不要再用其他脚本修改这台镜头的位置、Offset或Zoom。本脚本不处理V，窗口仍由你的windowcontroller.gd负责。

Map Bounds使用地图的世界坐标矩形：Position为左上角，Size为宽高。A地图1980×1080、左上角在(0,0)时，填写Position=(0,0)、Size=(1980,1080)。若Sprite2D默认Centered且Position=(0,0)、Scale=(1,1)、Offset=(0,0)，左上角应为(-990,-540)。地图缩放或移动后要填写变换后的实际范围。

| 检查器参数 | 默认值 | 含义 |
|---|---:|---|
| Camera Zoom | 1 | 镜头缩放倍率，使用此项而非原生Zoom |
| Follow Offset | (0,-40) | 平常跟随点相对玩家原点的偏移 |
| Follow Response | 10 | 跟随响应，越大越快 |
| Look Up Distance | 180 | 向上观察距离（世界像素） |
| Look Down Distance | 180 | 向下观察距离 |
...
范围必须为正宽高、轴对齐矩形。镜头自动忽略旋转并脱离父节点位移继承。未来切换房间时更新Map Bounds；当前不是自动识别地图图片边界。

已用800×450 SubViewport验证观察/回位、攀爬抑制、实际镜头中心、偏移地图和1200×900视窗下的小地图限制。正式项目仍应检查自己的地图坐标与视窗缩放配置。
