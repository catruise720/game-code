...
- 攀爬中←→允许有限横移；只横移保持第0帧，松键悬停，不自动回中。
...
玩家根节点的 Climbing 分组：Climb Speed=90（上下速度，像素/秒）、Climb Horizontal Speed=35（左右速度，像素/秒）、Climb Horizontal Limit=16（左右各允许偏移像素）。横移范围实际取该值和锁链 Detection Width/2 中较小者，避免手部横移超出检测区。保持碰撞检测，不穿过墙壁。两份脚本需要一起替换，新玩家脚本会读取新锁链的grab_top_y()/grab_bottom_y()。
